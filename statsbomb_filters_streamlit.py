...
st.set_page_config(
    page_title="Football match data | No more static football reports",
    page_icon=":soccer",
    #layout="wide",
...
st.title("No more static football reports")
...
with st.beta_expander("Shots and passes distribution by minute"):
    ...
    y_max_range = shots_data['type_name'].max() + 1
    # https://www.geeksforgeeks.org/create-grouped-bar-chart-using-altair-in-python/
    shots_chart = alt.Chart(shots_data, title="Shots by teams").mark_bar(size=3).encode( 
      alt.X('minute', axis=alt.Axis(title=None)), 
      alt.Y('type_name', axis=alt.Axis(grid=False), scale=alt.Scale(domain=(0, y_max_range), clamp=True)),  
      alt.Color('possession_team_name', legend=alt.Legend(title="Teams")),
      opacity=alt.value(0.9)
      ).properties(width=600)
    ...
    pass_chart = alt.Chart(pass_data, title="Passes by teams").mark_line(size=3).encode( 
      alt.X('minute', axis=alt.Axis(title=None)), 
      alt.Y('type_name', axis=alt.Axis(grid=False)),  
      alt.Color('possession_team_name', legend=alt.Legend(title="Teams")),
      opacity=alt.value(0.9)
    ...
    upper = pass_chart.encode(
    alt.X('minute', scale=alt.Scale(domain=brush))
    )
    
    lower = pass_chart.properties(
        height=60
    ).add_selection(brush)
    ...
    home_team = pass_data.loc[pass_data['possession_team_name'] == match['home_team']['home_team_name']]
    away_team = pass_data.loc[pass_data['possession_team_name'] == match['away_team']['away_team_name']]
    
    if st.button("Play"): 

        import time
        
        y_max_range = pass_data['type_name'].max() + 1
        x_max_range = pass_data['minute'].max() + 1
        source = pass_data
        chart = st.empty()
        st.button('Finish')
        ...
        for i in source.index:
            ...
            x = alt.Chart(data_to_be_added).mark_line(size=3).encode(
                alt.X('minute', axis=alt.Axis(title=None), scale=alt.Scale(domain=(0, x_max_range), clamp=True)), 
                alt.Y('type_name', axis=alt.Axis(grid=False), scale=alt.Scale(domain=(0, y_max_range), clamp=True)),
...
with st.beta_expander("Shots and passes distribution by 15-minute periods"):
...
with st.beta_expander("Expected goals (xG) distribution during the match"):
...
# pass pitch
with st.beta_expander("Check out shots and passes location on the pitch"):
    ...
    away_team_passes = passes.loc[passes['player_name'] == add_selectbox_player_pass]
    passes_stats = 'Good passes: ' + str(len(good_passes)) + ', bad passes: ' + str(len(bad_passes))
    plt.text(5,81,add_selectbox_player_pass + ' passes' + ' (total ' + str(len(away_team_passes)) + ') ' + str(passes_stats)) 
...
# https://github.com/altair-viz/altair/issues/1268
